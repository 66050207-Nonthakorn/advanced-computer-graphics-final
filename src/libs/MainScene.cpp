...
MainScene::MainScene() {
    ...
    face.material = MaterialManager::instance().get("default");

    SceneObject face2;
    face2.transform.setPosition({4, 2, 0});
    face2.transform.setScale({3, 3, 3});
    face2.mesh = MeshManager::instance().getFromFile("Models/face.obj");
    face2.material = MaterialManager::instance().get("face");
    ...
    this->directionalLight.intensity = 5.0f;
